...
class UpdateMetricsThread(threading.Thread):
    ...
    def _get_ps_output():
        try:
            procs = subprocess.check_output("/usr/bin/pgrep -f 'python -m ipykernel_launcher' -d','", shell=True).rstrip()
            output = subprocess.check_output("/usr/bin/ps -p {} -o rss,etimes,args --no-header".format(procs), shell=True)
            print output
        except:
            output = ''
...
def get_total_rss_in_gb(name):
    ...
    if not PS_OUTPUT:
        return 0
    return sum([float(line.split()[0])/1024/1024 for line in PS_OUTPUT])

def get_average_rss_in_gb(name):
    global PS_OUTPUT
    if not PS_OUTPUT:
        return 0
    return sum([float(line.split()[0])/1024/1024 for line in PS_OUTPUT]) / len(PS_OUTPUT)

def get_average_session_length_in_minutes(name):
    global PS_OUTPUT
    if not PS_OUTPUT:
        return 0
    return sum([float(line.split()[1])/60 for line in PS_OUTPUT]) / len(PS_OUTPUT)

def get_max_rss_in_gb(name):
    global PS_OUTPUT
    if not PS_OUTPUT:
        return 0
    return max([float(line.split()[0])/1024/1024 for line in PS_OUTPUT])

def get_max_session_length_in_minutes(name):
    global PS_OUTPUT
    if not PS_OUTPUT:
        return 0
    return max([float(line.split()[1])/60 for line in PS_OUTPUT])
...
def metric_init(params):
    ...
    descriptors = [
        ...
        'slope': 'both',
        'format': '%d',
        'description': 'Count of jupyter notebook processes',
        'groups': 'jupyter,questanalytics'},
        ...
        'slope': 'both',
        'format': '%.2f',
        'description': 'Total memory resident set size of jupyter notebook processes',
        'groups': 'jupyter,questanalytics'},
        ...
        'slope': 'both',
        'format': '%.2f',
        'description': 'Average memory resident set size of jupyter notebook processes',
        'groups': 'jupyter,questanalytics'},
        ...
        'slope': 'both',
        'format': '%.2f',
        'description': 'Max memory resident set size of a single jupyter notebook process',
        'groups': 'jupyter,questanalytics'},
        ...
        'slope': 'both',
        'format': '%.2f',
        'description': 'Average session length of jupyter notebook processes',
        'groups': 'jupyter,questanalytics'},
        ...
        'slope': 'both',
        'format': '%.2f',
        'description': 'Max session length of a single jupyter notebook process',
        'groups': 'jupyter,questanalytics'},
    ]
